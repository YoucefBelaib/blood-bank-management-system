...
export class DatabaseStorage implements IStorage {
  ...
  async getBloodRequestsByHospitalId(
    hospitalId: string
  ): Promise<BloodRequest[]> {
    const database = this.ensureDb();
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const donorsByBloodType = Array.from(bloodTypeMap.entries()).map(
      ([name, value]) => ({
        name,
        value,
      })
    ...
    const monthlyStats = months.map((month, index) => {
      ...
      const lastYearCount = allDonors.filter((donor) => {
        const date = new Date(donor.createdAt || Date.now());
        return (
          date.getFullYear() === currentYear - 1 && date.getMonth() === index
        );
      }).length;
    ...
    const totalHospitals = allHospitals.filter(
      (h) => h.status === "approved"
    ).length;
    const pendingHospitals = allHospitals.filter(
      (h) => h.status === "pending"
    ).length;
    const pendingRequests = allBloodRequests.filter(
      (r) => r.status === "pending"
    ).length;
