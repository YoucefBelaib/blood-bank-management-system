...
// Extended donor type with display status
...
const assignStatus = (donor: Donor): DonorStatus => {
  ...
  const hash = donor.fullName.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0);
...
const DonorsList: React.FC = () => {
  ...
  const donorsWithStatus: DonorWithStatus[] = useMemo(
    () => donors.map((donor) => ({ ...donor, status: assignStatus(donor) })),
    [donors]
  );

  // Filter donors based on search
  const filteredDonors = useMemo(() => {
    if (!searchQuery.trim()) return donorsWithStatus;
    
    const query = searchQuery.toLowerCase();
    return donorsWithStatus.filter(
      (donor) =>
        donor.fullName.toLowerCase().includes(query) ||
        donor.bloodType.toLowerCase().includes(query)
    );
  }, [donorsWithStatus, searchQuery]);

  const handleRowClick = (donor: DonorWithStatus) => {
